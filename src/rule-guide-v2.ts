...
function card(step:Step):Card{
  ...
  switch(step){
    ...
    case"J":{
      const iconPage=h.icon?iconPages[h.icon]:undefined;
      if(iconPage)pages.push(iconPage);
      return{title:"EXPERIENCE ICON EVENT",prompt:"On an Old Tile with an icon, go to that icon's event page and experience the event.",check:h.icon&&h.icon!=="None"?`Recorded icon: ${h.icon}.`:"No icon is recorded here; Step K may apply instead.",pages};
    }
...
function dice(n:number,s:number){
  const d=Array.from({length:n},()=>Math.floor(Math.random()*s)+1);
  return{d,total:d.reduce((a,b)=>a+b,0)};
}
...
function decorateSteps(){
  ...
  root.querySelectorAll<HTMLButtonElement>("[data-step]").forEach(button=>{
    const step=button.dataset.step as Step;
    if(!stepPrimaryPage[step])return;
    button.classList.add("guide-step");
    button.title=`What do I do now? Step ${step} — ${stepPages[step].join(", ")}`;
    button.innerHTML=`<b>${step}</b><span>${stepPrimaryPage[step]}</span><small>WHAT DO I DO?</small>`;
  });
}
...
function render(){
  ...
  const shownStep=previewStep??state.step;
  const c=card(shownStep);
  const sec=document.createElement("section");
  sec.id="miru-rule-guide";
  sec.className="rule-guide";
  sec.innerHTML=`
    <div class="rule-kicker"><span>WHAT DO I DO NOW?</span><b>STEP ${shownStep}</b></div>
    <h2>${c.title}</h2>
    <p class="rule-prompt">${c.prompt}</p>
    ${c.check?`<div class="rule-check">${c.check}</div>`:""}
    ${c.roll?`<div class="rule-roll"><div><span>ROLL / CHECK</span><b>${c.roll[0]}d${c.roll[1]} ${c.roll[2]}</b></div><button id="guide-roll">ROLL ${c.roll[0]}D${c.roll[1]}</button><output id="guide-roll-result">Ready</output></div>`:""}
    ${c.detail?`<details class="rule-detail"><summary>Result reference</summary><p>${c.detail}</p></details>`:""}
    <div class="rule-page-callout"><span>RULEBOOK</span><b>${stepPrimaryPage[shownStep]}</b></div>
    <details class="rule-pages"><summary>All references</summary><div>${c.pages.map(p=>`<span>${p}</span>`).join("")}</div><small>Printed page numbers in your MIRU rulebook PDF.</small></details>`;
  ...
  sec.querySelector("#guide-roll")?.addEventListener("click",()=>{
    if(!c.roll)return;
    const r=dice(c.roll[0],c.roll[1]);
    const out=sec.querySelector<HTMLOutputElement>("#guide-roll-result");
...
OBR.onReady(async()=>{
  ...
  document.addEventListener("click",event=>{
    const button=(event.target as Element|null)?.closest<HTMLButtonElement>("[data-step]");
    if(!button)return;
    const step=button.dataset.step as Step;
    if(!stepPrimaryPage[step])return;
    previewStep=step;
